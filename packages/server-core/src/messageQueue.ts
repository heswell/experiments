import { getFullRange, DataTypes } from '@heswell/data';
import { MessageOut, MessageTypeOut, RowMeta, UpdateMessage } from './serverTypes';
    ...
    if (vp === viewport) {
      if (type === ROWSET) {
        ...
        if (lo1 >= to || hi1 < from) {
          ...
          var overlaps = data.rows.filter((row: any[]) => row[IDX] >= low && row[IDX] < high);
...
const formatMessage = (msg: any) => ` type: ${msg.type} 
    rows: [${msg.data && msg.data.rows && msg.data.rows.map((row: any[]) => row[7])}]`;
