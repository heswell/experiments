...
import { Draggable } from "@heswell/grid";
...
const ColumnFilter = ({
  ...
  const filterSource = useRef(
    dataSourceFactory(dataSource, filterType, column, onDataCount)
  );
  ...
  const handleSearchText = (value) => {
    filterSource.current.filter(
      { type: STARTS_WITH, colName: "name", value },
      DataTypes.FILTER_DATA,
      true
    );
  };
  ...
  const getFilter = () => {
    ...
    return (
      <Draggable onDrag={moveFilter}>
        ...
        >
          {childFilter}
          <FilterCounts column={column} style={{ height: 50 }} stats={stats} />
        </FilterPanel>
      </Draggable>
    );
  };
  ...
  const className = cx("HeaderCell", "ColumnFilter", {
    active: isActive,
  ...
  return (
    ...
    <div
      ref={rootEl}
      className={className}
      style={{ padding: 0, width: column.width }}
    >
      <div className="filterButton" onClick={toggleFilterDisplay}>
      ...
      {isActive && (
        <div className="filterClearButton" onClick={clearFilter}>
