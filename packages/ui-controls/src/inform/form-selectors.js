...
export function nextField(state, evt=DEFAULT_DIRECTION, field=state.currentField, field2=field){
...
export const isSelect = (state, field=state.currentField) => 
  isComposite(field)
    ? field[state.compositeFieldIdx] === SELECT
    : field.type === SELECT

export const isCombo = (state, field=state.currentField) => 
  isComposite(field)
    ? isComboType(field.type[state.compositeFieldIdx])
    : isComboType(field.type)
