...
class BaseRowSet {
  ...
  setRange(range = this.range, useDelta = true, includeStats = false) {
    ...
    return {
      ...
      size: this.size,
      offset: this.offset,
      stats: includeStats ? this.stats : void 0
    };
  }
  currentRange() {
  ...
  select(idx, rangeSelect, keepExistingSelection) {
    ...
    if (filterSet) {
      ...
      const idxToIDX = (idx2) => sortSet[idx2][0];
      this.selectedRowsIDX = this.selected.rows.map(idxToIDX);
    }
    const updates = [];
    for (let i = 0; i < selected.length; i++) {
      const idx2 = selected[i];
      if (idx2 >= lo && idx2 < hi) {
        updates.push([idx2 + offset, SELECTED, 1]);
      }
    }
    for (let i = 0; i < deselected.length; i++) {
      const idx2 = deselected[i];
      if (idx2 >= lo && idx2 < hi) {
        updates.push([idx2 + offset, SELECTED, 0]);
      }
    }
    return updates;
  }
  ...
  selectNavigationSet(useFilter) {
    ...
    return useFilter ? [this.filterSet, NEXT_FILTER_IDX, FILTER_COUNT] : [this.sortSet, IDX_POINTER, COUNT];
  }
  ...
  getDistinctValuesForColumn(column) {
    ...
    const [, otherFilters] = splitFilterOnColumn(currentFilter, column);
...
class RowSet extends BaseRowSet {
  ...
  slice(lo, hi) {
    ...
    if (filterSet) {
      const filterMapper = typeof filterSet[0] === "number" ? (idx) => data[idx] : ([idx]) => data[idx];
      const results = [];
  ...
  sort(sortCols) {
    const sortSet = this.currentFilter === null ? this.sortSet : this.filterSet = sortableFilterSet(this.filterSet);
