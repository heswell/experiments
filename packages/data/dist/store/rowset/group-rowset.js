...
class GroupRowSet extends BaseRowSet {
  ...
  setRange(range, useDelta = true) {
    const [rowsInRange, idx] = !useDelta && range.lo === this.range.lo && range.hi === this.range.hi ? this.iter.currentRange() : this.iter.setRange(range, useDelta);
  ...
  setGroupState(groupState) {
    ...
    changes.forEach(([key, , isExpanded]) => {
      const { groupRows: groupRows2 } = this;
      if (key === "*") {
        this.toggleAll(isExpanded);
        this.currentLength = this.countVisibleRows(groupRows2, false);
      } else {
        const groupIdx = this.findGroupIdx(key);
        if (groupIdx !== -1) {
          if (isExpanded) {
            this.currentLength += this.expandGroup(groupIdx, groupRows2);
          } else {
            this.currentLength -= this.collapseGroup(groupIdx, groupRows2);
          }
        } else {
  ...
  toggleGroup(groupIdx, groupRows2, processChildGroups) {
    const { DEPTH, COUNT, FILTER_COUNT } = this.meta;
    let adjustment = 0;
    const groupRow = groupRows2[groupIdx];
    ...
    if (Math.abs(depth) === 1) {
      ...
      adjustment = processChildGroups(Math.abs(depth) - 1, groupIdx + 1, groupRows2, useFilter);
    }
    return adjustment;
  }
  countChildGroups(childDepth, startIdx, groupRows2, useFilter) {
    const { DEPTH, FILTER_COUNT } = this.meta;
    let adjustment = 0;
    for (let i = startIdx; i < groupRows2.length; i++) {
      const nextDepth = groupRows2[i][DEPTH];
      if (Math.abs(nextDepth) === childDepth) {
        if (!useFilter || groupRows2[i][FILTER_COUNT] > 0) {
          adjustment += 1;
        }
  ...
  collapseChildGroups(childDepth, startIdx, groupRows2, useFilter) {
    const { DEPTH, FILTER_COUNT } = this.meta;
    let adjustment = 0;
    for (let i = startIdx; i < groupRows2.length; i++) {
      const nextDepth = groupRows2[i][DEPTH];
      if (Math.abs(nextDepth) === childDepth) {
        if (!useFilter || groupRows2[i][FILTER_COUNT] > 0) {
          adjustment += 1;
          if (nextDepth > 0) {
            adjustment += this.collapseGroup(i, groupRows2);
          }
        }
      } else if (Math.abs(nextDepth) > childDepth) {
        break;
    ...
    for (let i = 0; i < groups.length; i++) {
      ...
      if (absDepth === 1) {
        ...
        if (this.aggregations.length) {
          aggregations = this.aggregations.map(([i2, a]) => [i2, a, 0]);
  ...
  update(rowIdx, updates) {
    ...
    if (groupUpdates) {
      for (let i = groupUpdates.length - 1; i >= 0; i--) {
        const [grpIdx, ...updates2] = groupUpdates[i];
        const rangeIdx2 = this.iter.getRangeIndexOfGroup(grpIdx);
        if (rangeIdx2 !== -1) {
          outgoingUpdates.push([lo + rangeIdx2 + offset, ...updates2]);
        }
      }
    }
  ...
  insert(newRowIdx, row) {
    ...
    iterator.refresh();
    let rangeIdx = allGroupsExist ? iterator.getRangeIndexOfRow(newRowIdx) : iterator.getRangeIndexOfGroup(newGroupIdx);
    if (rangeIdx !== -1) {
      result = { replace: true };
      if (newGroupIdx !== null) {
        this.currentLength += 1;
  ...
  findGroupIdx(groupKey) {
    const { groupRows: groupRows2, meta } = this;
    for (let i = 0; i < groupRows2.length; i++) {
      if (groupRows2[i][meta.KEY] === groupKey) {
        return i;
      }
    }
    return -1;
  ...
  sortGroupby(groupby) {
    ...
    this.groupRows.forEach((groupRow, i2) => {
      const depth2 = groupRow[DEPTH];
      const groupKey = groupRow[KEY];
      const absDepth = Math.abs(depth2);
      tracker.set(absDepth, i2, groupKey);
      groupRow[IDX] = i2;
      if (absDepth > 1) {
        groupRow[IDX_POINTER] = i2 + 1;
      }
  ...
  extendGroupby(groupby) {
    ...
    const filterFn = this.currentFilter ? filterPredicate(this.columnMap, this.currentFilter) : null;
    for (let i = 0; i < groups.length; i++) {
      const groupRow = groups[i];
      ...
      groupRow[meta.NEXT_FILTER_IDX] = void 0;
        ...
        if (absDepth > doomed) {
          ...
          if (absDepth === doomed + 1) {
            if (lastGroupIsDoomed) {
              ...
              groupRow[NEXT_FILTER_IDX] = useFilter ? lowestIdxPointer(groups, NEXT_FILTER_IDX, DEPTH, i + 1, absDepth - 1) : void 0;
  ...
  countVisibleRows(groupRows2, usingFilter = false) {
    const { DEPTH, COUNT, FILTER_COUNT } = this.meta;
    let count = 0;
    for (let i = 0, len = groupRows2.length; i < len; i++) {
      const zeroCount = usingFilter && groupRows2[i][FILTER_COUNT] === 0;
      if (!zeroCount) {
        count += 1;
      }
      const depth = groupRows2[i][DEPTH];
      if (depth < 0 || zeroCount) {
        while (i < len - 1 && Math.abs(groupRows2[i + 1][DEPTH]) < -depth) {
          i += 1;
        }
      } else if (depth === 1) {
        count += usingFilter ? groupRows2[i][FILTER_COUNT] : groupRows2[i][COUNT];
      }
    }
    return count;
  }
...
//# sourceMappingURL=group-rowset.js.map
