...
function next(groups, rows, grpIdx, rowIdx, navSet, NAV_IDX, NAV_COUNT, meta) {
  if (grpIdx === null) {
    ...
    if (depth === 1 && count !== 0 && (rowIdx === null || rowIdx < count - 1)) {
      ...
      do {
        grpIdx += 1;
      } while (grpIdx < groups.length && (Math.abs(groups[grpIdx][meta.DEPTH]) < absDepth || getCount(groups[grpIdx], NAV_COUNT) === 0));
      if (grpIdx >= groups.length) {
        return NO_RESULT;
      } else {
        return [groups[grpIdx], grpIdx, null];
...
export {
  BACKWARDS,
  FORWARDS,
  GroupIterator as default
};
//# sourceMappingURL=groupIterator.js.map
