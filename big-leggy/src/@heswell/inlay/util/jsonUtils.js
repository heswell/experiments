...
import {addPaths, adjustPath, followPath} from './pathUtils';
	...
	for (var op in options){
		...
		switch(op){
			...
			case 'move':
				...
				if (opts.before){
					//onsole.log(`transform: move before ` + opts.before.$path);
					nodeBeforeWhichToInsert = opts.before.$path;
				}
				else if (opts.after){
					//onsole.log(`transform: move after ` + opts.after.$path);
			...
			case 'wrap':
                pos = opts.pos;
				nodeToBeRemoved = opts.source.$path ? opts.source : undefined;;
			...
			case 'insert':
				...
				if (opts.container){
					//onsole.log(`transform: insert into container ` + opts.container.$path);
...
function insert(json, node, container, before, after, nodeSize){
	...
	if (nodeSize){
		...
		var {flexGrow,flexShrink,resizeable} = node.$size;
		node.$size = {flexGrow,flexShrink,flexBasis:nodeSize,resizeable};
	}
...
function insertAfter(json, path, node, nodeSize){
    ...
    for (var property in json){
        if (property === 'content'){
            ...
            if (paths.length === 1){
            	if (nodeSize){
		       		var dimension = json.flexDirection === 'column' ? 'height' : 'width';
            ...
            if (paths.length === 2 && content[idx].content.length === 2 && content[idx].type === 'Tabs'){
                ...
                if (alignedFlexBoxes(newJson, remainingTwin)){
                    ...
                    remainingTwin.content.forEach(c => {
                        c.$size.flexBasis = c.$clientRect[newJson.flexDirection === 'column' ? 'height' :'width'];
                    });
        ...
        if (property === 'content'){
            ...
            if (paths.length === 2 && content[idx].content.length === 2){
            	...
            	if (alignedFlexBoxes(newJson, remainingTwin)){
            		...
            		//TODO should clone here
    	        	remainingTwin.$size = parentOfTwins.$size;  
                    // shouldn't have to do this, this should go inside size
                    remainingTwin.resizeable = parentOfTwins.resizeable;
    	        	containerRemoved = 1;
            	}

            }
            else if (paths.length === 1){
                var doomedNode = newJson.content[idx];
            	newJson.content.splice(idx,1);
                // This is getting too complicated: do we really need to maintain $size here ?
                // reallocate the flexBasis of the removed node between the remaning flexible nodes
                if (doomedNode.$size){
                    newJson.content = reassignBoxSize(newJson.content, doomedNode.$size.flexBasis);
                }
...
function wrap(pos, source, target){
    ...
    if (target.type === 'Container'){
        var {$clientRect,$size} = target;
        target = {...target.content[0], $clientRect, $size};
    }
    ...
    if (pos.position.EastOrWest && pos.width){
        ...
        var flexBasis;
        if (dim && target.$clientRect){
            flexBasis = Math.round(target.$clientRect[dim]/2);
        }
        else {
            flexBasis = Math.round(target.$size.flexBasis/2);
        }
    ...
    return {type, flexDirection, activeTab, dragContainer, $size, content, resizeable:target.resizeable};
}
...
function alignedFlexBoxes(c1, c2){
	return c1.type === 'FlexBox' && c2.type === 'FlexBox' &&
		   c1.flexDirection === c2.flexDirection;
}

function reassignBoxSize(content, flexBasis){

    var total = content.reduce((total,c) => total + c.$size.flexBasis,0); 
    var newTotal = total + flexBasis;

    return content.map(component => {

        var value = component.$size.flexBasis;
        var frac = value / total;

        var dolly = shallowCloneObject(component);
        dolly.$size.flexBasis = Math.round(frac * newTotal);
        return dolly;
