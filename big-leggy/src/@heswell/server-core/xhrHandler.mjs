...
export function handleRequest(request, response){
    ...
    request.on('end', () => {
        ...
        if (message.type === 'connect'){
            ...
            console.log(`after calling the handler, queue contains ${JSON.stringify(connection.queue.length)} records`);
    ...
    clients.forEach(clientId => {
        ...
        if (queue.length){
            if (response){
                ...
                console.log(`We have updates and more than ${Frequency.TIMEOUT/1000} seconds (TIMEOUT_PERIOD) since we communicated with this client`);
                deadClient = true;
            }
            else {
                console.log(`processXhrHandler: client ${clientId} has ${queue.length} updates queued, nor response available, wait for next client HB `);
            }

        }
        else if (currentTime - timestamp > Frequency.HEARTBEAT){
            console.log(`more than ${Frequency.HEARTBEAT/1000} seconds (HEARTBEAT_FREQUENCY) since we communicated with this client`);
            _client_xhr_connections[clientId].response = null;
            ...
            if (response){
                ...
                console.log(`more than ${Frequency.TIMEOUT/1000} seconds (TIMEOUT_PERIOD) since we communicated with this client`);
