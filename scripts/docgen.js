...
const componentFolder = "./packages/grid/src";
...
function parseComponent(component, filename) {
  ...
  const spacedName = (shortname + SPACE).slice(0,30)
  try {
    const componentInfo = reactDocs.parse(component);
    componentInfo.filename = shortname;
    pushComponent(componentInfo);
    console.error(`${spacedName} generated`);
  } catch(e){
    console.error(`${spacedName} ${e.message}`);
  }
}
/**
