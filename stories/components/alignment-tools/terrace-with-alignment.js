...
import { Action, FlexBox } from '@heswell/layout';
...
const Input = props => {
  ...
  return <StyledInput placeholder={props.placeholder} onClick={handleClick} value={props.value} onChange={props.onChange}/>
}
...
const SizePanel = ({dispatch, style, selectedLayoutModel}) => {
  ...
  const handleChange = (name, value) => {
    const parsedValue = isNaN(parseInt(value,10))
      ? value
      : parseInt(value,10);
  ...
  return (
    <div style={style}>
      <Input placeholder="width" value={width || ''} onChange={e => handleChange('width', e.target.value)}/>
      <Input placeholder="height" value={height || ''}  onChange={e => handleChange('height', e.target.value)}/>
      <Input placeholder="flex" value={flex || ''}  onChange={e => handleChange('flex', e.target.value)}/>
    </div>
  )
}
